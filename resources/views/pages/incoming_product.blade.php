...
@section('content')
    <div class="overflow-x-auto m-20">
        <h1 class="mb-5 text-center text-3xl font-mono font-bold text-info">Barang Masuk</h1>
        ...
        <table class="table table-zebra">
            ...
            <tbody>
                @foreach ($incoming as $in)
                    <tr>
                        <th>{{ $in->TransaksiID }}</th>
                        <td>{{ $in->SalesTransaksiID }}</td>
                        <td>{{ $in->Tanggal }}</td>
                        <td>{{ $in->ItemID }}</td>
                        <td>{{ $in->Jumlah }}</td>
                        <td>{{ $in->HargaPerItem }}</td>
                        <td class="text-center">
                            <button class="btn btn-outline btn-primary cursor-pointer"
                                onclick="editModal{{ $in->IncomingID }}.showModal()">Edit</button>
                            <dialog id="editModal{{ $in->IncomingID }}" class="modal">
                                <div class="modal-box">
                                    <form method="dialog">
                                        <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
                                    </form>
                                    <h3 class="font-bold text-lg">Edit Data</h3>
                                    <form method="POST" action="/incoming/{{ $in->IncomingID }}/update" class="my-2">
                                        @csrf
                                        @method('PUT')
                                        <div class="mx-auto">
                                            <div class="label">
                                            ...
                                            <select name="TransaksiID" class="select mb-2 select-primary w-full" required>
                                                <option value="{{ $in->salesTransaction->TransaksiID }}" selected>
                                                    {{ $in->salesTransaction->TransaksiID . ' - ' . $in->salesTransaction->Tanggal . ' - ' . $in->salesTransaction->TotalHarga . ' - ' . $in->salesTransaction->customer->Nama }}
                                                </option>
                                                @foreach ($sales as $s)
                                            ...
                                            <select name="SalesTransaksiID" class="select my-2 select-primary w-full"
                                                required>
                                                <option value="{{ $in->TransaksiID }}" selected>
                                                    {{ $in->TransaksiID . ' - ' . $in->purchaseTransaction->Tanggal . ' - ' . $in->purchaseTransaction->TotalHarga . ' - ' . $in->purchaseTransaction->supplier->Nama }}
                                                </option>
                                            ...
                                            <select name="ItemID" class="select my-2 select-primary w-full" required>
                                                <option value="{{ $in->item->ItemID }}" selected>
                                                    {{ $in->item->ItemID . ' - ' . $in->item->Nama }}
                                                </option>
                                            ...
                                            <input type="number" minlength="1" name="HargaPerItem"
                                                ...
                                                required value="{{ $in->HargaPerItem }}" />
                                            <div class="label ">
                            ...
                            <button class="btn btn-outline btn-error cursor-pointer"
                                onclick="{{ 'deleteModal' . $in->IncomingID }}.showModal()">Delete</button>
                            <dialog id="{{ 'deleteModal' . $in->IncomingID }}" class="modal ">
                                <div class="modal-box">
                                    <form method="dialog">
                                        <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
                                    </form>
                                    ...
                                    <form action="/incoming/{{ $in->IncomingID }}/delete" method="post">
                                        @csrf
                                        @method('DELETE')
        ...
        <dialog id="insertModal" class="modal">
            <div class="modal-box">
                ...
                <form method="POST" action="/incoming/insert" class="my-2">
                    @csrf
                    @method('POST')
                    <div class="mx-auto">
                        <div class="label">
